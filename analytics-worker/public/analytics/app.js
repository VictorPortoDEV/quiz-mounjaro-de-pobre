...
const short=id=>id.replaceAll('-','').slice(0,8).toUpperCase();
const setText=(id,value)=>{$(`#${id}`).textContent=value};

function renderFunnel(data){const values=[['Entradas',data.entries],['Iniciaram Quiz',data.quiz_started],['Concluíram Quiz',data.quiz_completed],['VSL',data.vsl_started],['CTA',data.cta]];$('#funnel').replaceChildren(...values.map(([name,value],index)=>{const item=document.createElement('div');item.className='funnel-step';const rate=index&&values[index-1][1]?Math.round(value/values[index-1][1]*100):100;const nameEl=document.createElement('span');nameEl.textContent=name;const count=document.createElement('strong');count.textContent=value;const percent=document.createElement('small');percent.textContent=`${rate}%`;item.append(nameEl,count,percent);return item}))}

function progressCell(session){const wrapper=document.createElement('div');if(session.current_page==='quiz')wrapper.textContent=`Etapa ${session.current_step||1}/${session.quiz_total_steps||'—'}`;else{const value=session.vsl_progress||0;wrapper.textContent=`${value}%`;const bar=document.createElement('div');bar.className='progress-bar';const fill=document.createElement('i');fill.style.width=`${value}%`;bar.append(fill);wrapper.append(bar)}return wrapper}

function renderSessions(data){const tbody=$('#sessions');tbody.replaceChildren();for(const session of data.sessions){const tr=document.createElement('tr');tr.addEventListener('click',()=>openDetail(session.session_id));const values=[short(session.session_id),session.current_page,progressCell(session),duration(data.now-session.started_at),session.utm_source||'Direto',session.utm_campaign||'—',ago(session.last_seen_at,data.now)];values.forEach((value,index)=>{const td=document.createElement('td');if(value instanceof Node)td.append(value);else td.textContent=value;if(index===0)td.className='session';if(index===1)td.innerHTML=`<span class="tag">${session.current_page}</span>`;if(index===6&&data.now-session.last_seen_at<=30)td.className='recent';tr.append(td)});tbody.append(tr)}$('#empty').hidden=data.sessions.length>0}
